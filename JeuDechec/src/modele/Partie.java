...
public class Partie extends Observable {
	...
	public boolean isCurrentPlayersKingInCheckMate(Piece king){
		...
		/* si le roi peut bouger,  v�rifier si ses positions ne sont pas toutes compromises */
		ArrayList<Position> kingMoves = king.getAvailablePositions();
		ArrayList<Piece> indirectPredators = new ArrayList<>(); // pi�ces qui peuvent le manger a toutes ses positions
		for(Position p : kingMoves){
			Roi tmp = new Roi(this.whitesTurn, this.plateau, p); // on simule le d�placement
			ArrayList<Piece> tmpPredators = whoCanEatTarget(oponentPieces, tmp);
			if(tmpPredators.size() != 0){ // le roi peut etre mang� peut importe ou il va
				for(Piece predator : tmpPredators){ // on ajoute les pr�dateurs <=> les pieces qui PEUVENT le manger dans le futur
		...
		if(indirectPredators.size() != 0){ // si au moins une pi�ce peut le manger a ses positions
			kingCantMove = true;
		...
		/* on recherche quelles pi�ces parmis celles de l'adversaire peuvent manger le Roi du joueur courant */
		...
		/* Maintenant, parmis les pi�ces du joueur, les quelles peuvent manger le pr�dateur ? */	
		ArrayList<Piece> directPredatorsEliminated = new ArrayList<>();
		
		for(Piece predator : directPredators){ // pour chaque pr�dateur DIRECT...
			for(Piece p : playersPieces){ // on regarde pour toutes les pi�ces du joueur si...
				for(Position pos : p.getAvailablePositions()){ // chaques positions disponnibles des pi�ces...
					// ... qui �liminent (mange) un pr�dateur (pas d�ja �limin�)
					if(pos.equals(predator.position) && !directPredatorsEliminated.contains(predator)){ // 
						directPredatorsEliminated.add(predator);
					}
				}
			}
		}
		
		boolean kingCanBeEaten = directPredators.size() != directPredatorsEliminated.size();
		if( kingCanBeEaten && kingCantMove) // si au moin UN predateur est encore envie, echec et mat
			return true;
		return false;
	}
	...
	public ArrayList<Piece> whoCanEatTarget(ArrayList<Piece> pieces,Piece target) {
		...
		return ret; // la liste des pr�dateur de target
	}
